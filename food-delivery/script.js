...
(function initNavbar() {
  ...
  const panelBurger = document.getElementById('panelBurger');
  ...
  // Wire up SVGator player so the icon morphs hamburger ↔ X
  let svgPlayer = null;
  const svgEl = document.getElementById('eRzKqZPLYfF1');
  if (svgEl && svgEl.svgatorPlayer) {
    svgEl.svgatorPlayer.ready(player => {
      svgPlayer = player;
      player.seek(0);  // ensure we start at hamburger state
      player.pause();
    });
  }
  ...
  function openMenu() {
    ...
    if (svgPlayer) svgPlayer.play();   // hamburger → X
    overlay.classList.add('is-open');
    overlay.setAttribute('aria-hidden', 'false');
    document.body.style.overflow = 'hidden';
    gsap.killTweensOf([panel, navItems]);
    gsap.to(panel, { x: 0, duration: 0.45, ease: 'power3.out',
      onComplete: () => { animating = false; }
    });
    gsap.to(navItems, { x: 0, opacity: 1, duration: 0.35, stagger: 0.06, ease: 'power2.out', delay: 0.15 });
  }
  ...
  function closeMenu() {
    ...
    if (svgPlayer) svgPlayer.reverse(); // X → hamburger
    gsap.killTweensOf([panel, navItems]);
    gsap.to(navItems, { x: 30, opacity: 0, duration: 0.2, stagger: 0.04, ease: 'power2.in' });
    gsap.to(panel, {
      x: '100%', duration: 0.4, ease: 'power3.in', delay: 0.04,
      onComplete: () => {
  ...
  panelBurger.addEventListener('click', closeMenu);
