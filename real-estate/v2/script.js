...
function initStatBarsObserver() {
  ...
  new IntersectionObserver(entries => {
    entries.forEach(entry => {
      ...
      // Read the target width from the inline CSS custom property
      const targetW = bar.style.getPropertyValue('--w').trim() || '70%';

      // Double rAF: first frame ensures element is painted at width:0,
      // second frame applies the target width so transition can animate.
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
    ...
    // Observe the section, fire once per bar
    document.getElementById('stats') || bars[0]
  );

  // Alternatively observe each bar individually for precision
